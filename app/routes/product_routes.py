...
def shop():
    if 'email' in session:
        ...
        return render_template('product/product_shop.html', products=products, cart_count=cart_count)
    else:
...
def search():
    if 'email' in session:
        ...
        if session['email'] == 'admin@grocify.com':
            return render_template('product/product_search_admin.html', products=products, query=query, cart_count=cart_count)
        else:
            # Render the template and pass the product data to it
            return render_template('product/product_search.html', products=products, query=query, cart_count=cart_count)
    else:
        return redirect(url_for('auth_routes.signin'))